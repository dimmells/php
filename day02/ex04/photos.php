...
function download($tag_img, $address) {
	preg_match_all("/(?<=src=\").*?(?=\")/i", $tag_img, $url, PREG_PATTERN_ORDER);
	 foreach ($url as $el) {
	 	foreach ($el as $elem) {
		 	if (preg_match("/http/i", $elem)) {
		 		preg_match("/(?<=\/\/).*/i", $address, $www);
		 		$address = $www[0] . "/";
		 		if (!is_dir($address))
...
if ($argv[1]) {
	...
	preg_match_all("/<img.*?>/i", $source, $matches, PREG_PATTERN_ORDER);
	foreach ($matches as $value) {
		foreach ($value as $tag_img) {
			download($tag_img, $argv[1]);
		}
	}
	preg_match_all("/(?<=url=\").*?(?=\")/i", $source, $matches, PREG_PATTERN_ORDER);
	foreach ($matches as $value) {
		foreach ($value as $tag_img) {
